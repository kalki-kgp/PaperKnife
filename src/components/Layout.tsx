...
const categoryColors: Record<ToolCategory, { bg: string, text: string, hover: string, iconBg: string }> = {
  Edit: { bg: 'bg-terracotta-50 dark:bg-terracotta-900/20', text: 'text-terracotta-500', hover: 'hover:bg-terracotta-50 dark:hover:bg-terracotta-900/10', iconBg: 'bg-terracotta-100 dark:bg-terracotta-900/30' },
  ...
  Signatures: { bg: 'bg-teal-50 dark:bg-teal-900/20', text: 'text-teal-600', hover: 'hover:bg-teal-50 dark:hover:bg-teal-900/10', iconBg: 'bg-teal-100 dark:bg-teal-900/30' }
}
...
export default function Layout({ children, tools, onFileDrop, viewMode, variant, onChangeVariant }: LayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-[#FFF3F0] dark:bg-black text-gray-900 dark:text-zinc-100 transition-colors duration-300`}>
      ...
      {isDragging && (
        <div className="fixed inset-0 z-[200] bg-terracotta-500/10 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="bg-white dark:bg-zinc-900 p-12 rounded-[3rem] shadow-2xl border-4 border-dashed border-terracotta-500 animate-in zoom-in duration-300">
            <UploadIcon size={64} className="text-terracotta-500 animate-bounce" />
            <p className="mt-4 font-black uppercase tracking-widest text-terracotta-500 text-center text-sm">Drop PDF to start</p>
          </div>
        </div>
      )}
        ...
        <header className="flex items-center justify-between px-4 md:px-8 h-16 md:h-20 border-b border-orange-100 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/80 backdrop-blur-md sticky top-0 z-[100]">
          <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
            ...
            <div className="relative min-w-0" ref={dropdownRef}>
              <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`flex items-center gap-2 px-2 md:px-3 py-1.5 rounded-xl transition-all font-black text-[10px] md:text-sm uppercase tracking-widest min-w-0 ${isDropdownOpen ? 'bg-terracotta-500 text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-12 gap-8 mb-12">
              ...
              <div className="col-span-2 md:col-span-6 space-y-4">
                <Link to="/" className="flex items-center gap-2.5 text-gray-900 dark:text-white group w-fit">
                  <PaperKnifeLogo size={22} iconColor="#E68A73" partColor="currentColor" />
                  <span className="font-bold tracking-tight text-lg group-hover:text-terracotta-500 transition-colors">PaperKnife</span>
                </Link>
        ...
        <div className="p-6 h-full flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <HistoryIcon className="text-terracotta-500" size={24} />
            ...
            {activity.length === 0 ? (<div className="text-center py-20 opacity-40"><p className="text-xs font-bold uppercase tracking_widest text-gray-500">No recent files</p></div>) : (
              activity.map((item) => (
                <div key={item.id} className="p-4 bg-gray-50 dark:bg-zinc-900/50 rounded-2xl border border-gray-100 dark:border-zinc-800 group relative">
                  ...
                  <div className="flex items-center justify-between text-[9px] text-gray-400 font-bold"><span>{new Date(item.timestamp).toLocaleTimeString()}</span>{item.resultUrl && (<a href={item.resultUrl} download={item.name} className="text-terracotta-500 hover:underline flex items-center gap-1"><DownloadIcon size={10} /> Redownload</a>)}</div>
                </div>
              ))
            )}
          </div>
